...
namespace PEIU.Service.WebApiService.Controllers
{
    ...
    public class ContractorController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetContractors(string aggregatorgroupid = null)
        {
            try
            {
                IEnumerable<VwContractoruser> source = null;
        ...
        {
            List<VwContractorsite> source = null;
            try
            {
                if (HttpContext.User.IsInRole(UserRoleTypes.Supervisor))
                {
                    ...
                    {
                        source = new List<VwContractorsite>();
                        foreach (VwContractoruser user in _accountContext.VwContractorusers.Where(x => x.AggGroupId == aggregatorgroupid))
                        {
                ...
                else if (HttpContext.User.IsInRole(UserRoleTypes.Aggregator))
                {
                    ...
                    {
                        foreach (VwContractoruser user in _accountContext.VwContractorusers.Where(x => x.AggGroupId == groupId))
                        {
                            source.AddRange(_accountContext.VwContractorsites.Where(x => x.UserId != null && x.UserId == user.UserId));
                        }
                    }
