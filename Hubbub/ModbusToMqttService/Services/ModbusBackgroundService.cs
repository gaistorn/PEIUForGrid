...
{
    public class ModbusBackgroundService : BackgroundHostService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
                ...
                foreach (var x in modbus.GroupPoints)
                {
                    ...
                    if (DateTime.Now > dtMap[x.GroupId])
                    {
                        ...
                        HashEntry[] hashEntries = null;
                         JObject parentModel = modbusFactory.ReadModbusToJson(SiteId, DeviceIndex, x, out hashEntries);
                        if (parentModel == null)
                        {
                            dtMap[x.GroupId] = DateTime.Now.Add(TimeSpan.FromSeconds(x.RetryIntervalSec));
                            continue;
                        ...
                        string topic = $"hubbub/{SiteId}/{parentModel["normalizedeviceid"].ToString()}/AI";
            ...
            Console.WriteLine("ModbusBackgroundService - ExecuteAsync End");
